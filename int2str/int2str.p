...
#всего копеек
	$cop(100*$amount)
#округляем и преобразуем к целому числу, чтобы исключить радости double
	$cop(^math:round($cop))
#все расчёты в целых числах
	$rub(^math:trunc($cop/100))
	$cop($cop%100)
	$result[^int2str($rub)[m;upper] ^currency_name[$rub;$drub] $cop ^currency_name[$cop;$dcop]]


#############################
@currency_name[value;names][_value]
# Для числей от 10 до 20 нужно писать рублей/копеек, иначе в зависимости от последнего знака
^if($value%100 < 20){ $_value($value%100) }{ $_value($value%10) }
$result[$names.$_value]
